...
class NewsService {
        ...
        items.forEach((item, index) => {
          ...
          if (title && description) {
            articles.push({
              ...
              url: link,
              urlToImage: this.extractImageFromDescription(description) || this.getFallbackImage(),
  ...
  // ─── Dynamically Generate Trending Topics from Articles ─────
  async fetchTrendingTopics(articles: ExternalNewsArticle[]): Promise<{
    ...
    articles.forEach(article => {
      ...
      text.split(/\s+/).forEach(word => {
        if (word.length > 3 && !['https', 'match', 'cricket'].includes(word)) {
  ...
  async fetchAllNews(): Promise<{
    ...
    trending: { id: string; name: string; count: number; trend: 'up' | 'down' | 'stable' }[];
  }> {
    try {
      ...
      const combined = this.deduplicateArticles([...newsApiArticles, ...rssArticles])
